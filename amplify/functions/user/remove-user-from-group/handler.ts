import { 
  removeUserFromGroup as removeUserFromGroupOperation, 
  getUserPoolId, 
  getCognitoClient,
  formatUser,
  FormattedUser
...
export const handler = async (event: AppSyncEvent): Promise<Response> => {
  ...
  try {
    ...
    const result = await removeUserFromGroupOperation({
      userPoolId,
      username: userId,
      groupName
    }, cognitoClient);
    ...
    console.error('Error in removeUserFromGroup Lambda:', error);
    return {
      success: false,
